...
- Python 3.11+
- A Google Ads account (a Manager Account is optional; only needed to reach several accounts through one login)
- A Google Cloud project with OAuth credentials and Google Ads API access (the wizard walks you through it, see [Google Cloud project setup](/self-hosting/google-cloud-project)). No developer token since September 2026.
...
2. **API access and MCC account ID**: both optional prompts now; access is applied for on the Cloud project, the MCC only matters with a Manager Account
...
<Tip>
Prefer zero setup? [AdLoop Cloud](/quickstart/cloud) runs the same tools without a Google Cloud project or an API access application.
</Tip>
