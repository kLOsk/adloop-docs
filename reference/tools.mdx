...
| `health_check` | Read | Test AdLoop connectivity — checks OAuth token, GA4 API, Google Ads API, |
...
| `draft_create_conversion_action` | Write | Draft a new Google Ads ConversionAction — returns a PREVIEW. |
...
| `draft_remove_conversion_action` | Write | Draft a REMOVAL of a ConversionAction — returns a PREVIEW. |
...
| `draft_update_conversion_action` | Write | Draft a partial UPDATE of an existing ConversionAction — returns PREVIEW. |
...
| `update_responsive_search_ad` | Write | Update mutable fields on an existing RSA in place — returns a PREVIEW. |
...
| Tool | Type | What it does |
|------|------|--------------|
| `draft_reddit_ad` | Write | Draft a Reddit ad: creates a post on the profile, then the ad (PAUSED) — PREVIEW. |
| `draft_reddit_ad_group` | Write | Draft a new Reddit ad group (created PAUSED) — returns a PREVIEW. |
| `draft_reddit_campaign` | Write | Draft a new Reddit campaign (created PAUSED) — returns a PREVIEW. |
| `enable_reddit_entity` | Write | Draft enabling (configured_status=ACTIVE) a Reddit campaign, ad group or ad — PREVIEW. |
| `estimate_reddit_ad_group` | Read | Audience size, delivery estimate and Reddit's suggested bid for a planned ad group — read-only. |
| `get_reddit_account_history` | Read | Who changed what in the Reddit ad account: field, before/after, member, time. |
| `get_reddit_ad_groups` | Read | List Reddit ad groups (budget, bid, pixel, weekly schedule, targeting summary), optionally per campaign. |
| `get_reddit_ads` | Read | List Reddit ads with status, rejection_reason, post and click URL. |
| `get_reddit_campaigns` | Read | List Reddit campaigns with status, objective, budget mode and bids. |
| `get_reddit_performance` | Read | Reddit Ads performance: spend, impressions, clicks, CTR, CPC, conversions, CPA, ROAS. |
| `get_reddit_pixels` | Read | Reddit pixels of the account and when each event (purchase, sign_up, lead, ...) last fired. |
| `list_reddit_accounts` | Read | List Reddit businesses and ad accounts the connected Reddit user can access. |
| `list_reddit_funding_instruments` | Read | Funding instruments (billing) and posting profiles of a Reddit ad account. |
| `pause_reddit_entity` | Write | Draft pausing a Reddit campaign, ad group or ad — returns a PREVIEW. |
| `remove_reddit_entity` | Destructive | Draft ARCHIVING a Reddit campaign, ad group or ad — irreversible, PREVIEW. |
| `run_reddit_report` | Read | Run a custom Reddit Ads report for metrics get_reddit_performance omits. |
| `search_reddit_targeting` | Read | Look up targeting options for draft_reddit_ad_group. |
| `update_reddit_ad` | Write | Draft changes to a Reddit ad — name, landing URL (click_url), comments on/off. |
| `update_reddit_ad_group` | Write | Draft changes to a Reddit ad group — budget, bid, run dates, weekly schedule, targeting. |
| `update_reddit_campaign` | Write | Draft changes to a Reddit campaign — name, run dates, spend cap, and (CBO only) budget/bid/schedule. |
